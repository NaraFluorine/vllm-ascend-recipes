...
3. **Validate**: `pnpm validate` — fails fast on schema errors, including the cross-field interlock rules (features referenced by `opt_in_features` / `{{name}}` placeholders, `scenario.strategy` ⊆ `compatible_strategies`, `%%CONFIG%%` markers having a matching `extra_config` key or feature).
4. **Preview**: `pnpm dev` — open the page at `/{provider}/{model}`.
5. **Translate** to `models/zh/{Provider}/{Model-Name}.yaml` with the **same field structure** (1:1 with `en/`, only descriptions differ). If a model is missing in `zh/`, the site falls back to the English content.
...
| `features` | no | object | Upstream feature declarations (`label` / `description` / `args` / `env`) — boolean toggles are driven from here |
| `opt_in_features` | no | array | Feature keys that default **off** (must reference `features`) — upstream aligned |
| `variants` | no | object | Upstream variants; `default` variant is required |
| `compatible_strategies` | no | array | Deployment strategies, interlocked with `scenarios[].strategy` (e.g. `single_node_A2`, `single_node_A3`, `pd_cluster`) |
| `hardware_overrides` | no | object | Per-hardware `{extra_args, extra_env}` overrides (e.g. `ascend_a3`) — upstream aligned |
...
| `date_updated` | no | Latest material recipe change (`YYYY-MM-DD`) — upstream aligned |
| `difficulty` | no | `beginner` \| `intermediate` \| `advanced` — upstream aligned |
| `tasks` | no | Task types, e.g. `[text]`, `[text, image]` |
| `performance_headline` | no | One-line performance summary shown on the card |
| `related_recipes` | no | List of related `org/repo` ids — upstream aligned |
...
### `config_params` and `{{name}}` placeholders

`config_params` holds editable value params (defaults = tutorial baseline). Any `{{name}}` inside a step is replaced with the current value:
...
Boolean toggles are **not** declared in `config_params` — they live in `features` and are substituted the same way (`{{name}}` renders the feature's `args`/`env` when on, `flag_when_false` when off). The rendered command highlights each substituted flag in the same color as its Config chip.

### `features` and `opt_in_features`
...
features:
  spec_decoding:
    ...
    args: ["--speculative-config", '{"num_speculative_tokens": 1}']
  flashcomm1:
    label: FlashComm1
    env:
...
opt_in_features:        # these default OFF; features absent here default ON
  - prefix_caching
```

Feature keys prefer the upstream convention (`spec_decoding`, not `mtp`). `args` or `env` is required.

### `variants` and `compatible_strategies`

```yaml
variants:
  default:                                # required whenever variants is present
    precision: W8A8                       # upstream enum: bf16|fp8|nvfp4|fp4|int4|int8|awq|gptq|mxfp4; W8A8 is the Ascend extension
    vram_minimum_gb: 341                  # ceil(params × bytes_per_param × 1.2)
    description: W8A8 weights on Atlas 800 A2/A3
  w8a8_mtp:
    model_id: Eco-Tech/DeepSeek-V4-Flash-w8a8-mtp   # quantized variant in another repo
    precision: W8A8
    vram_minimum_gb: 341

compatible_strategies:                    # interlocked with scenarios[].strategy
  - single_node_A2
  - single_node_A3
  - pd_cluster
```

`hardware_overrides` / `strategy_overrides` follow the same `{extra_args, extra_env}` shape and are optional.
